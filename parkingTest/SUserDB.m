...
- (void) saveUser:(SUser *) user {
    ...
    if (user.description1) {
        [keys appendString:@"description1,"];
        [values appendString:@"?,"];
        [arguments addObject:user.description1];
    }
    [keys appendString:@")"];
    [values appendString:@")"];
    ...
    NSLog(@"%@ %@  %@ ",query,user.name,arguments);
...
- (void) mergeWithUser:(SUser *) user {
    ...
    if (user.description1) {
        [temp appendFormat:@" description = '%@',",user.description1];
    }
    [temp appendString:@")"];
...
- (NSArray *) findWithUid:(NSString *) uid limit:(int) limit {
	...
	while ([rs next]) {
        ...
        user.description1 = [rs stringForColumn:@"description1"];
